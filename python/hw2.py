# Jeremiah Mathew
# https://www.w3schools.com/python/gloss_python_set_length.asp#:~:text=To%20determine%20how%20many%20items,use%20the%20len()%20method. 
...
def remove_range(alist, min, max):
   ...
   *** Important: your code must use comprehensions and should not be more than
   two lines of code including the return statement ***
    """
    remove_final = [i for i in alist if i < min or i > max]
    return remove_final
...
def word_count(strings):
    ...
    *** Important: your code must use comprehensions and should not be more than two
    ...
    """
    final_count = {val: strings.count(val) for val in strings}
    return final_count
...
def friend_list(friend_dictionary):
    ...
    """
    new_friend_dictionary = {}
    for key, value in friend_dictionary.items():
        if value in new_friend_dictionary: # bidirectional in both statements so if you get one friend you add the other (depends on who's the key and who's the value) 
            new_friend_dictionary[value].append(key)
        else:
            new_friend_dictionary[value] = [key]
        if key in new_friend_dictionary:
            new_friend_dictionary[key].append(value)
        else:
             new_friend_dictionary[key] = [value]
    return new_friend_dictionary
